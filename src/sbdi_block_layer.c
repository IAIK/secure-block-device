...
void bl_aes_cmac(const sbdi_t *sbdi, const sbdi_block_t *blk, sbdi_tag_t tag)
{
  ...
  sbdi_bl_aes_cmac(ctx, &ctr, sizeof(sbdi_ctr_128b_t), msg, mlen, C);
}
...
static sbdi_error_t bl_read_mngt_block(sbdi_t *sbdi, sbdi_block_t *mng)
{
  ...
  printf("... Reading management block %d (phy): \n", mng->idx);
  mt_al_print_hex_buffer(tag, sizeof(sbdi_tag_t));
  printf("\n");
...
/*!
 * \brief Encrypts and writes a specific management block to the data store.
 *
 ...
 */
static sbdi_error_t bl_encrypt_write_mngt(sbdi_t *sbdi, const sbdi_block_t *mng,
    sbdi_tag_t mng_tag)
{
  assert(sbdi && mng);
  assert(sizeof(sbdi_ctr_128b_t) == SBDI_BLOCK_CTR_SIZE);
  assert(mng != &sbdi->write_store[0]);
  ...
  SBDI_BLOCK_SIZE, mng_tag, 2, &mng->idx, sizeof(uint32_t));
  sbdi->write_store[0].idx = mng->idx;
  // TODO if this gets only partially written then there is a big problem!
  return sbdi_bl_write_block(sbdi, &sbdi->write_store[0], SBDI_BLOCK_SIZE);
}
...
static sbdi_error_t bl_ensure_mngt_blocks_exist(sbdi_t *sbdi, uint32_t l)
{
  ...
  while (s < mng_blk_nbr) {
    ...
    sbdi->write_store[1].idx = sbdi_bl_mng_idx_to_mng_phy(s);
    vprf(sbdi->ctx, *sbdi->write_store[1].data, 1, &sbdi->g_ctr,
        sizeof(sbdi_ctr_128b_t));
    sbdi_ctr_128b_inc(&sbdi->g_ctr);
    SBDI_ERR_CHK(bl_encrypt_write_mngt(sbdi, &sbdi->write_store[1], mng_tag));
    SBDI_ERR_CHK(
        bl_mt_sbdi_err_conv(mt_add(sbdi->mt, mng_tag, sizeof(sbdi_tag_t))));
    printf("... Creating management block %d (phy): \n",
        sbdi->write_store[1].idx);
    mt_al_print_hex_buffer(mng_tag, sizeof(sbdi_tag_t));
    printf("\n");
    s += 1;
  }
  return SBDI_SUCCESS;
    ...
    sbdi_block_t *mng)
{
  sbdi_tag_t mng_tag;
  memset(mng_tag, 0, sizeof(sbdi_tag_t));
  SBDI_ERR_CHK(bl_encrypt_write_mngt(sbdi, mng, mng_tag));
